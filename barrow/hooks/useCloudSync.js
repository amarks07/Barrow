...
// essentially-empty error — message "{}" , "[]", or blank. Rather than
// show that literally, fall back to something a user can act on. Check the
...
export function useCloudSync({
  ...
  // someone lands back on the app from a "reset your password" email link.
  // While true, the UI shows a "set a new password" prompt instead of
  ...
  useEffect(() => {
    if (!supabase || !session || recoveryMode) return;
    let cancelled = false;
    ...
    (async () => {
      ...
      if (cloudHasData && (!localHasData || window.confirm(
  ...
  useEffect(() => {
    ...
    pushTimeoutRef.current = setTimeout(async () => {
      setStatus("syncing");
      const { error: pushError } = await supabase.from("profiles").update({
        first_name: profile.firstName,
        last_name: profile.lastName,
        username: profile.username,
        picture_url: profile.pictureUrl,
