...
export function RoutineDetailView({
  ...
  const supersets = routine.supersets || [];
  const hasAnyGroup = supersets.length > 0;

  const usedDates = useMemo(() => {
    return Object.entries(workouts)
      .filter(([, dayWorkouts]) => dayWorkouts.some((w) => (w.routineIds || []).includes(routine.id)))
      .map(([dateKey]) => dateKey)
      .sort((a, b) => (a < b ? 1 : -1))
      .slice(0, 5);
  }, [workouts, routine]);
  ...
  const rowRuns = useMemo(() => {
    ...
    return runInfo(routine.exerciseIds, groupKey);
  }, [routine.exerciseIds, supersets]);

  const { draggingGroupIds, dragOffsetY, shiftForIndex, refCallback, makeRowGesture } = useDragReorderList({
    ids: routine.exerciseIds,
    groupOf: (id) => {
      const gi = groupIndexOf(supersets, id);
    ...
    <View style={{ flex: 1, backgroundColor: tokens.bg }}>
      ...
      >
        ...
        <View className="flex-1">
          <EditableTitle
            value={routine.name}
            onChange={(name) => onRename(routine.id, name)}
            textStyle={{ fontFamily: FONT_DISPLAY, fontSize: 19 }}
          />
        </View>
        <IconBtn label="Share routine" onPress={() => setShowShare(true)}>
          <Share2 size={17} color={tokens.text} />
        </IconBtn>
        <ConfirmDeleteIconButton onConfirm={onDelete} label="Delete routine" size={16} />
      </View>
      <ScrollView style={{ flex: 1, paddingHorizontal: 20 }} contentContainerStyle={{ paddingTop: 16, paddingBottom: 24 + insets.bottom }}>
        ...
        {(routine.exerciseIds.length >= 2 || supersetMode || removeSupersetMode) && (
      ...
      {showPicker && (
        <ExercisePicker
          ...
          setExerciseView={() => {}}
          alreadyPicked={routine.exerciseIds}
          onPick={(ex) => onAddExercise(routine.id, ex.id)}
      ...
      {showShare && <ShareRoutineModal routine={routine} exercises={exercises} onClose={() => setShowShare(false)} />}
    </View>
  );
}
